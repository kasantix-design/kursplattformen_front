...
export default function KjøpKurs() {
  ...
  const handleStripe = async () => {
    const res = await fetch("/api/betaling/stripe", {
      method: "POST",
      headers: {
    ...
    else alert("Klarte ikke å starte Stripe-betaling")
  }
  ...
  return (
    <div className="content-area">
      <div className="content-box">
        ...
        <p>
          <strong>Pris: {kurs.pris} NOK</strong>
        </p>

        <div style={{ display: "flex", gap: "1rem" }}>
          <button className="btn" onClick={handleStripe}>
            Kjøp med Stripe
          </button>
          <button className="btn" onClick={handleVipps}>
            Kjøp med Vipps
          </button>
        </div>
      </div>
